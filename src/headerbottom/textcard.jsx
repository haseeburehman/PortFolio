...
const TextCard = () => {
  ...
  const texts = ["a Front-end Developer", "a Coder", "a Designer"];
  ...
  useEffect(() => {
    ...
    const typingDelay = isDeleting ? 30 : 100;
    ...
    const interval = setInterval(() => {
      if (isDeleting) {
        ...
        setDisplayedText((prev) => texts[textIndex].slice(0, charIndex + 1));
  ...
  }, [charIndex, textIndex, displayedText, isDeleting]);

  useEffect(() => {
    if (isDeleting) {
      const delay = 20000; // Delay before starting to delete
  ...
  return (
    <div className={styles.textcard__card}>
      <span className={styles.textcard__title}>
        <pre>Hello! I'm </pre>
        <pre className={styles.name}>Haseeb ur Rehman</pre>
        <div className={styles.divdisplay}>
          <pre className={styles.displaytext}>{displayedText}</pre>
        </div>
      </span>

      <p className={styles.textcard__subtitle}>
        I am a professional front-end website developer with experience working
        in various companies. I have a strong expertise in the latest
        technologies used for front-end development. Throughout my career, I've
        completed numerous websites, showcasing my skills in creating responsive
        and user-friendly designs. You can explore my work in the project
        section, where you'll find detailed descriptions and code.
      </p>
      <span className={styles.span}>
        Let's bring your ideas to life! Feel free to reach out to me through my
        social accounts.
